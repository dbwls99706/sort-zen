...
- [2026-06-16] Phase 19(T160~T164) 완료: Google Play Games 선택적 로그인 + "최고 도달 단계" 리더보드. 점수 순수로직(leaderboard.ts)·GameServicesManager(안드/웹스텁)·userStore 인증상태(persist v2)·메뉴/설정 UI·클리어 시 점수제출·Expo config plugin(APP_ID 주입). react-native-google-leaderboards-and-achievements v2.2.1 도입. verify 통과(145 tests). 남은 T165(Play Console 실ID·외부계정)/T166(Saved Games 동기화)는 사람 작업·후속
- [2026-06-24] T170 완료: 인앱 업데이트(Phase 20). UpdateManager(안드)+웹스텁으로 expo-in-app-updates 캡슐화 — 시작 시 1회 checkAndStartUpdate로 Play가 최신 여부 판단·우선순위에 따라 즉시/유연 자동선택. _layout init 연결, 동적 import로 웹 번들 분리, 예외/비안드로이드 안전 no-op. config plugin 불필요(autolinking). verify 통과(153 tests). vc15부터 적용
- [2026-07-21] T180 완료: Expo SDK 54 업그레이드로 대상 API 36(Android 16) 충족 — Play 2026-08-31 요구사항 ①. RN 0.81.5/Reanimated 4/Skia 2.2.12 정렬, Kotlin 2.1.20, withPlayGamesServices 프로퍼티 동기화, SharedValue 타입 교체. verify 통과(153 tests) + prebuild로 gradle.properties targetSdk 36 확인
- [2026-07-21] T181 완료: expo-iap 4.6.0 도입으로 Play Billing 9.1.0 충족 — Play 2026-08-31 요구사항 ②(8.0.0+, 권장 9). SubscriptionManager OpenIAP API 마이그레이션(공개 API 불변), 광고 SDK 16.4.0 상향, SPEC/docs 스택 표기 갱신. verify 통과 + prebuild로 BILLING 권한·openiap-google 2.4.1 주입 확인
- [2026-07-22] T182 완료: 모듈 스코프 Dimensions 제거 — targetSdk 36의 대화면 방향 고정 무시 대응. Background(그라데이션·보케), ClearModal(컨페티 원점)은 useWindowDimensions로, Onboarding 폭은 alignSelf stretch로 교체. 게임 화면은 이미 useWindowDimensions 사용 중이라 변경 없음. verify 통과(153 tests)
