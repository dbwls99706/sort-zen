...
- [x] T120 만족스러운 클리어 공유 — ClearModal 공유 버튼(OS 공유 시트, 레벨/이동수/별점 텍스트). 결과 카드 이미지/클립 내보내기는 view-shot 의존성 필요 시 후속
- [x] T121 데일리 스트릭 가시적 훅 강화 — ClearModal(보상 직후)에 데일리 진행 ⚡ + 스트릭 🔥 라인 노출, 메뉴 카드와 이중 노출
